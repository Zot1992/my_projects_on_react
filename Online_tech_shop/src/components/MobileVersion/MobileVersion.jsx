...
const MobileVersion = ({
    ...
    return (
        <div className="block md:hidden max-w-screen-sm  mx-auto mt-2 mb-2 p-4 text-center bg-gray-50 rounded-lg shadow-lg">
            {/* Верхние кнопки */}
            <div className='flex items-center justify-center'>
                {/* Кнопка для открытия корзины и избранного*/}
                <button data-testid="cart-button"
                    className="mr-3  cursor-pointer p-2 rounded-2xl text-gray-600 hover:bg-white "
                    onClick={openCart}><span className='flex items-center justify-center'>
                ...
                <button data-testid="favorites-button"
                    className=" cursor-pointer p-2 rounded-2xl text-gray-600 hover:bg-white"
                    onClick={openFavorites}><span className='flex items-center justify-center'>
            ...
            {isCartOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center"
                    onClick={(e) => {
                        if (e.target.classList.contains('dont_close') || e.target.closest('.dont_close')) return;
                        closeCart();
                    }}>
                    <div className="bg-white p-4 rounded-lg max-w-sm mx-auto overflow-auto max-h-[80vh] shadow-lg">
                        <ShoppingCart
                        ...
                        />
                        <button className="mt-4 w-full cursor-pointer bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-400"
                            onClick={closeCart}>Закрыть</button>
                    </div>
                </div>
            )}
            ...
            {/* Модалка избранного */}
            {isFavoritesOpen && (
                <div data-testid="favorites-modal" className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center"
                    onClick={(e) => {
                        if (e.target.classList.contains('remove-button') || e.target.closest('.remove-button')) return;
                        closeFavorites();
                    }}>
                    <div className="bg-white p-4 rounded-lg max-w-sm mx-auto overflow-auto max-h-[80vh] shadow-lg">
                        <Favorites
                        ...
                        />
                        <button className="mt-4 w-full cursor-pointer bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-400"
                            onClick={closeFavorites}>Закрыть</button>
                    </div>
                </div>
            )}
